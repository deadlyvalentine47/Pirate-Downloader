# 👣 Guide 2: Optimization & UX Polish

## Phase 5: Optimization (The robust "Speed Boost") 🏎️

### Step 14: Dynamic Chunking & Robust Queue
To solve the "Straggler Effect" (slow threads buffering forever) and "Missing Chunks" (network drops), we use a **Work Queue** (`VecDeque`) with a **completion tracker**.

**Key Features**:
*   **Queue**: If a chunk fails, it goes back to the queue (infinite retries).
*   **Speed Limit**: If a thread is < 300KB/s, we kill it, and the chunk goes back to the queue.
*   **Completion Tracking**: Threads don't exit when the queue is empty—they wait until ALL chunks are marked complete.

**Replace the ENTIRE content of `src-tauri/src/lib.rs` with this final code:**
...
async fn download_file(
    ...
    println!("File Size: {} bytes ({} MB)", total_size, total_size / 1024 / 1024);
    ...
    let path = PathBuf::from(&filepath);
    {
        ...
        file.seek(SeekFrom::Start(total_size - 1)).map_err(|e| e.to_string())?;
    ...
    for i in 0..total_chunks { chunks_vec.push(i); }
    let chunk_queue = Arc::new(tokio::sync::Mutex::new(std::collections::VecDeque::from(chunks_vec)));
    ...
    let speed_stats = Arc::new(tokio::sync::Mutex::new((0u64, 0.0f64, f64::MAX, std::time::Instant::now())));

    let actual_threads = if threads > 0 { threads } else { 8 };
    let mut handles = vec![];
    println!("Starting {} threads (Dynamic Mode) | Chunk Size: {} MB", actual_threads, chunk_size / 1024 / 1024);
    ...
    for _ in 0..actual_threads {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                if completed.load(Ordering::Relaxed) >= total_chunks {
                    break;
                }
                ...
                while attempts < 5 && !success { // 5 Retries per grab
                    attempts += 1;
                    if let Ok(mut response) = client.get(&url).header(RANGE, range_header.clone()).send().await {
                         if response.status().is_success() {
                            if writer.seek(SeekFrom::Start(start)).await.is_ok() {
                                ...
                                while let Some(chunk) = response.chunk().await.unwrap_or(None) {
                                    // SPEED ENFORCEMENT: > 300KB/s
                                    let elapsed = attempt_start.elapsed().as_secs_f64();
                                    if elapsed > 3.0 { 
                                        let speed = (bytes_this_attempt as f64 / 1024.0) / elapsed; 
                                        if speed < 300.0 { // Too slow? Kill it.
                                            chunk_ok = false; 
                                            break; 
                                        }
                                    }

                                    if writer.write_all(&chunk).await.is_err() { chunk_ok = false; break; }
                                    ...
                                    if (new / (1024*1024)) > (old / (1024*1024)) {
                                         ...
                                         let elapsed_stat = last_time.elapsed().as_secs_f64();
                                         if elapsed_stat >= 1.0 {
                                             let speed_mb = ((new - last_bytes) as f64 / 1048576.0) / elapsed_stat;
                                             if speed_mb > peak { peak = speed_mb; }
                                             if speed_mb < min && speed_mb > 0.0 { min = speed_mb; } 
                                             println!("Status: {} / {} MB ({:.2} MB/s)", new/1048576, total_size/1048576, speed_mb);
                                ...
                                if chunk_ok { 
                                    success = true; 
                                    completed.fetch_add(1, Ordering::Relaxed); // Mark as done
                                }
                            }
                        }
                    }
                    if !success { tokio::time::sleep(std::time::Duration::from_millis(200 * attempts)).await; }
                }

                if !success {
                    // CRITICAL: Push back to queue to be retried by ANY thread
                    println!("Chunk {} failed! Pushing back to queue.", idx);
                    let mut q = queue.lock().await;
                    q.push_back(idx); 
                }
            }
    ...
    for handle in handles { handle.await.map_err(|e| e.to_string())?; }
    ...
    println!("Average Speed: {:.2} MB/s", avg_speed);
    {
        ...
        println!("Min Speed: {:.2} MB/s", if min == f64::MAX { 0.0 } else { min });
    }
    ...
    if final_bytes < total_size {
        return Err(format!("Download FAILED: {} / {} bytes. Retry.", final_bytes, total_size));
    }
...
async fn get_file_details(url: String) -> Result<(String, u64), String> {
    ...
    if !response.status().is_success() { return Err(format!("Server returned error: {}", response.status())); }
    ...
    if let Some(disp) = response.headers().get(CONTENT_DISPOSITION) {
        if let Ok(disp_str) = disp.to_str() {
            if let Some(name_part) = disp_str.split("filename=").nth(1) {
                filename = name_part.trim().trim_matches('"').trim_matches('\'').to_string();
            }
        }
    } else {
        if let Ok(parsed_url) = url::Url::parse(&url) {
            if let Some(segments) = parsed_url.path_segments() {
                if let Some(last) = segments.last() {
                    if !last.is_empty() { filename = last.to_string(); }
                }
            }
        }
    }
...
## Step 16: React UI Update

Use the `App.tsx` logic as previously discussed (auto-history, file browsing, etc.).
